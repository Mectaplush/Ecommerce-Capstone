  ...
  return (
    <>
      ...
      >
          ...
          {links.map((link, index) => {
            return (
              <button
                onClick={() => {
                  setActiveLink(index);
                  dispatch(toggleComponent(link.title));
                }}
                key={index}
                className={`hover:bg-gray-200 w-full transition-all duration-300 rounded-md cursor-pointer px-3 py-2 flex items-center gap-2 ${
        ...
        <button onClick={handleLogout} className="text-white rounded-md cursor-pointer flex items-center px-3 py-2 gap-2 bg-red-gradient">
          <LogOut /> Logout
        </button>
      </aside>
    </>
