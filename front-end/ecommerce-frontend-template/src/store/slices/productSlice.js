...
export const fetchProductsWithAI = createAsyncThunk(
  ...
  async (userPrompt, thunkAPI) => {
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to fetch products with AI");
      return thunkAPI.rejectWithValue(error.response?.data?.message || "Failed to fetch products with AI");
    }
  }
);
