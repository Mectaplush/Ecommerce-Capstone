...
const Orders = () => {
  ...
  const filterOrders = myOrders.filter((o) => {
    const st = o.order_status || o.status || o.orderStatus;
    ...
    <>
      <div className="min-h-screen pt-20">
        <div className="container mx-auto px-4 py-8">
          ...
          {/* ORDER LIST  */}
          {fetchingOrders && <div>Loading orders...</div>}
          {!fetchingOrders && filterOrders.length === 0 ? (
            <div className="text-center glass-panel max-w-md mx-auto">
              ...
              <p className="text-muted-foreground">
                {statusFilter === "All"
                  ...
                  : `No orders found with status "${statusFilter}" found.`}
              </p>
            </div>
          ) : (
              ...
              {filterOrders.map((order) => {
                return (
                  <div key={order.id} className="glass-card">
                    {/* ORDER HEADER  */}
                    <div className="flex flex-col md:flex-row mg:items-center md:justify-between mb-6 space-y-4 md:space-y-0">
                      <div>
                    ...
                    <div className="flex flex-wrap gap-3 mt-6 pt-6 border-t border-[hsla(var(--glass-border))]">
                      ...
                      <>
                        ...
                        {/* <Link to={`/product/${order.order_items[0].product_id}`}
                          className="px-4 py-2 glass-card hover:glow-on-hover animate-smooth text-sm">
                            Write Review
                          </Link>
                          <Link className="px-4 py-2 glass-card hover:glow-on-hover animate-smooth text-sm">
                            Reorder
                          </Link> */}
                      </>
                      {/* )} */}
